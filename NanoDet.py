...
class nanodet():
    def __init__(self, input_shape=320, prob_threshold=0.4, iou_threshold=0.3):
        ...
        self.net = cv2.dnn.readNet('data/nanodet.onnx')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Object Detection using NanoDet in OPENCV')
    ...
    net = nanodet(input_shape=320, prob_threshold=args.confThreshold, iou_threshold=args.nmsThreshold)
    ...
    if args.fourcc == 0:
        ...
        out = cv2.VideoWriter('NanoDet.avi', fourcc, 20.0, (int(cap.get(3)), int(cap.get(4))))
